...
import { getStandings, fetchTeamsInfo, fetchLobbyMatches } from './DataHandler.js'

function getStandingsFromMB(id){
    // const standingsURL = `https://app.masterblaster.gg/api/scores/standings/${id || div3AId}`;
    return getStandings(id); //await fetch(standingsURL).catch(err => console.log(err)).then(data => data.json());
}
function fetchTeamsInfoFromMB(id){
    // const tournamentInfoURL = `https://app.masterblaster.gg/api/Tournament/cached/${id || div3AId}`;
    return fetchTeamsInfo(id); //await fetch(tournamentInfoURL).catch(err => console.log(err)).then(data => data.json());
}
function fetchLobbyMatchesFromMB(id){
    // const matchesURL = `https://app.masterblaster.gg/api/matches/tournament/${id || div3A}?includeStreamedMatchesOnly=false`;
    return fetchLobbyMatches(id); //await fetch(matchesURL).catch(err => console.log(err)).then(data => data.json());
}
...
export async function getTeamsInfo(id){
    var teamsInfo = (await fetchTeamsInfoFromMB(id)).tournamentTeams.map(data => data.team).map(team => {
...
export async function getStandingTable(id, teamsInfo){
    ...
    const standings = (await getStandingsFromMB(id)).map(team => {
